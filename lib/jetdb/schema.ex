defmodule Jetdb.Schema do
  ...
  defp is_user_table(columns, column_positions) do
    (Enum.at(columns, Enum.at(column_positions, 1)) &&& 0x00FFFFFF) == 1 && (Enum.at(columns, Enum.at(column_positions, 3)) &&& 0x80000002) == 0
  end

  # is &&& 0x7F needed for table definition page?
  defp parse_columns(columns, column_positions) do
    [
      Enum.at(columns, Enum.at(column_positions, 0)), # table definition page
      Enum.at(columns, Enum.at(column_positions, 1)) &&& 0x00FFFFFF, # type
      Enum.at(columns, Enum.at(column_positions, 2)), # name
      Enum.at(columns, Enum.at(column_positions, 3)) &&& 0x80000002 # flags
    ]
  end

  def read_schema(data_file) do
    schema_page = Enum.at(data_file, 2) # schema or catalog is page 2
    ...
    tables = Enum.filter(rows, &is_user_table(&1, column_positions)) |> Enum.map(&parse_columns(&1, column_positions))
